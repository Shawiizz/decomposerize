...
const decomposerize = (input: string, configuration: Configuration = {}): string => {
  ...
  const config = Object.assign({}, defaultConfiguration, configuration) as Required<Configuration>;
  ...
  if (config.services.length > 0) {
    ...
    (composeJson as any).services = filteredServices;
  }
  ...
  // Networks
  Object.entries((composeJson as any).networks || []).forEach(([networkName, network]: [string, any]) => {
  ...
  // Volumes
  Object.entries((composeJson as any).volumes || []).forEach(([volumeName, volume]: [string, any]) => {
  ...
  Object.entries((composeJson as any).services || []).forEach(([serviceName, service]: [string, any]) => {
  ...
  Object.entries((composeJson as any).services || []).forEach(([, service]: [string, any]) => {
